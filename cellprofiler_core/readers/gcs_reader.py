...
class GcsReader(ImageIOReader):
  ...
  def supports_format(cls, image_file, allow_open=False, volume=False):
      ...
      ."""
      logging.debug(
          "!!!DEBUG LOGGING!!! gcs_reader.supports_format({})".format(image_file))
  ...
  def get_reader(self, volume=False):
    ...
    logging.debug(
        f"!!!DEBUG LOGGING!!! gcs_reader.get_reader file._url {self.file._url})")
    path_to_image = self.download_blob(self.file._url)
    if self._reader is None or volume != self._volume:
        logging.debug(
            "!!!DEBUG LOGGING!!! gcs_reader.get_reader for path: {}".format(self.file._url))
        if volume:
  ...
  def decode_gcs_url(self, url):
    logging.debug(
        f"!!!DEBUG LOGGING!!! gcs_reader.decode_gcs_url - url {url})")
    p = urlparse(url)
    path = p.path.split('/', 1)
    bucket, file_path = p.netloc, path[1:][0]
    logging.debug(
        f"!!!DEBUG LOGGING!!! gcs_reader.decode_gcs_url - bucket {bucket} filepath {file_path})")
